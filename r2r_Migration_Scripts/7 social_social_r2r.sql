...
left outer join [social].[social] c (nolock) on a.social_id = c.r2r_social_id
where --a.dealer_id in (48625, 2990, 2991) and 
c.social_id is null and 
lower(b.accountstatus) = 'active' and a.dealer_id in (select r2r_dealer_id from auto_portal.portal.account with (nolock) where r2r_dealer_id is not null and lower(accountstatus) = 'active')
