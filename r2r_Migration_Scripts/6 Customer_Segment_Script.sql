...
left outer join customer.segments c (nolock) on a.contact_list_id = c.r2r_contactlist_id
where c.segment_id is null and lower(b.accountstatus) = 'active' and a.dealer_id in (select r2r_dealer_id from auto_portal.portal.account with (nolock) where r2r_dealer_id is not null and lower(accountstatus) = 'active')
...
from [18.216.140.206].[r2r_admin].[owner].[contact_list]  a with (nolock)
inner join auto_portal.portal.account b (nolock) on a.dealer_id = b.r2r_dealer_id
left outer join [list].[lists] c (nolock) on a.contact_list_id = c.r2r_custsegment_id
where c.list_id is null
and lower(b.accountstatus) = 'active' and a.dealer_id in (select r2r_dealer_id from auto_portal.portal.account with (nolock) where r2r_dealer_id is not null and lower(accountstatus) = 'active')
