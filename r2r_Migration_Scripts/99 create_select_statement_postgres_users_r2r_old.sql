use r2r_portal
select distinct 
	'insert into public.r2ruser_del (
	_id
	,accountname
	,createdate
	,firstname
	...
	+ isnull(c.dealer_name,'') +''','''
	+ cast(convert(date,Isnull(CreatedDt, getdate())) as varchar(50)) + ''','''
	+ isnull(FirstName,'') +''','''
	...
	+ cast(isnull(PhoneNumberConfirmed,'') as varchar(5))+''',null,'''
	+ cast(isnull(TwoFactorEnabled,'') as varchar(5)) +''','''
	+ isnull(Email,'')+''','''
	+ isnull(UserGuid,'')+''','''
	+ isnull(Cast(c.dealer_id as varchar(25)),'')+''','''
	+ isnull(ForgotPasswordOtp,'')+''')'
from r2r_portal.dbo.users a (nolock)
inner join portal.user_claims b (nolock) on a.userid = b.[user_id]
inner join portal.dealer c (nolock) on b.claim_value = c.dealer_id
where c.dealer_id in (48428) and b.claim_type_id = 1
...
--select * from r2r_portal.dbo.users a (nolock)