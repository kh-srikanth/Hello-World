...
left outer join email.campaigns c with (nolock) on a.campaign_id = c.r2r_campaign_id
where a.media_type = 'Email' and c.campaign_id is null  and lower(b.accountstatus) = 'active'
and a.dealer_id in (select r2r_dealer_id from auto_portal.portal.account with (nolock) where r2r_dealer_id is not null and lower(accountstatus) = 'active')
...
from [18.216.140.206].[r2r_cm].campaign.campaign a (nolock)
inner join auto_portal.[portal].[account] b with (nolock) on a.dealer_id = b.r2r_dealer_id
left outer join sms.campaigns c with (nolock) on a.campaign_id = c.r2r_campaign_id
where a.media_type = 'SMS' and c.campaign_id is null and lower(b.accountstatus) = 'active'
and a.dealer_id in (select r2r_dealer_id from auto_portal.portal.account with (nolock) where r2r_dealer_id is not null and lower(accountstatus) = 'active')
...
from [18.216.140.206].[r2r_cm].campaign.campaign a (nolock)
inner join auto_portal.[portal].[account] b with (nolock) on a.dealer_id = b.r2r_dealer_id
left outer join push.campaigns c with (nolock) on a.campaign_id = c.r2r_campaign_id
where a.media_type = 'push' and c.campaign_id is null and lower(b.accountstatus) = 'active'
and a.dealer_id in (select r2r_dealer_id from auto_portal.portal.account with (nolock) where r2r_dealer_id is not null and lower(accountstatus) = 'active')
...
from [18.216.140.206].[r2r_cm].campaign.campaign a (nolock)
inner join auto_portal.[portal].[account] b with (nolock) on a.dealer_id = b.r2r_dealer_id
left outer join social.campaigns c with (nolock) on a.campaign_id = c.r2r_campaign_id
where a.media_type = 'social' and c.campaign_id is null and lower(b.accountstatus) = 'active'
and a.dealer_id in (select r2r_dealer_id from auto_portal.portal.account with (nolock) where r2r_dealer_id is not null and lower(accountstatus) = 'active')
