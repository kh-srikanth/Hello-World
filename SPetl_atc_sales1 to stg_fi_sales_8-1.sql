...
begin
	...
	select distinct 
		...
		,[file_process_detail_id]			--as [file_log_detail_id]		--c
		,0 as is_err_rec
    ...
    inner join master.dealer md with(nolock) on 
	    a.[DV Dealer ID] = md.natural_key
	where 
		a.file_process_detail_id = @file_log_detail_id
	...
	(
     ...
     select
		...
		,[Model]
		,[Model Number]
		,(case when upper([New/Used]) = 'USED' then 'U' else 'N' end) as [New/Used]
		,[Exterior Color]
		,[Stock Number]
		,[Transmission]
		,[Engine Configuration]
       ...
       from  
			#temp with(nolock) 	   		
	  -- order by 
	    --   a.[etl_fi_sale_id]					--etl_fi_sale_id  PK auto increment?
	Update 
		a
	set
	...
	inner join #temp b on 
		a.etl_fi_sale_id = b.etl_atc_sale_id			---#temp.etl_fi_sale_id is from?
	where 
		b.is_err_rec = 1
    ...
    select 
         etl_atc_sale_id
   	    ...
   	    ,err_description
   	    ,vin
		,[Vendor Dealer ID]				-- parent_dealer_id
